((window) => {
  if (window.welpodron && window.welpodron.templater) {
    ...
    type CalculatorFormConfigType = {};

    type CalculatorFormPropsType = {
      element: HTMLElement;
      config?: CalculatorFormConfigType;
    };
    ...
    class CalculatorForm {
      ...
      constructor({ element, config = {} }: CalculatorFormPropsType) {
      ...
      handleFormInput = async (event: Event) => {
        ...
        this.timeout = setTimeout(async () => {
          ...
          try {
            ...
            const result: _BitrixResponse | null = await basket.set({
              args: data,
              event,
            });
            ...
            if (result.status === "error") {
              ...
              if (error.code === FIELD_VALIDATION_ERROR_CODE) {
                const field = this.element.querySelectorAll(
                  `[name="${error.customData}"]`
                )[0] as
                  | HTMLInputElement
                  | HTMLTextAreaElement
                  | HTMLSelectElement;
